...
* Multi-class classification estimates Pr(count=k) for user-selected discrete counts and a
  final "tail" bucket; a Tweedie regressor models only the tail, and the total forecast is
...
def parse_counts_arg(raw: str) -> List[int]:
    parts = [part.strip() for part in raw.split(",") if part.strip()]
    if not parts:
        raise ValueError("Counts list cannot be empty.")
    try:
        counts = sorted(set(int(p) for p in parts))
    except ValueError as exc:
        raise ValueError("Counts must be comma-separated integers.") from exc
    if any(c < 0 for c in counts):
        raise ValueError("Counts must be non-negative integers.")
    if 0 not in counts:
        raise ValueError("Counts list must include 0 to model outage-free events.")
    return counts
...
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Optional[List[Tuple[int, int]]]]:
    num_locations = outages.shape[1]
    hour_sin, hour_cos, dow_sin, dow_cos = trig_cache
    active_lags = [lag for lag in lag_list if lag <= lookback_steps]
    active_out_sum = [w for w in OUT_ROLL_SUM_WINDOWS if w <= lookback_steps]
    active_out_max = [w for w in OUT_ROLL_MAX_WINDOWS if w <= lookback_steps]
    active_weather_roll_max = [w for w in WEATHER_ROLL_MAX_WINDOWS if w <= lookback_steps]
    active_weather_roll_sum = [w for w in WEATHER_ROLL_SUM_WINDOWS if w <= lookback_steps]
    hazard_feature_span = 2 + len(active_weather_roll_max) + len(active_weather_roll_sum) + 1
    total_dim = (
        1
        + len(active_lags)
        + len(active_out_sum)
        + len(active_out_max)
        + 1
        + len(hazard_idx) * hazard_feature_span
        + 4
        + 1
    )
    ...
    for t in time_indices:
        ...
        for loc in range(num_locations):
            ...
            if not np.isfinite(target_val):
                continue
            row = np.empty(total_dim, dtype=np.float32)
            pos = 0
            row[pos] = float(hist_len)
            pos += 1
            ...
            for window in active_out_max:
                ...
                if start_idx >= 0:
                    vals = outages[start_idx:t, loc]
                    finite = np.where(np.isfinite(vals), vals, np.nan)
                    val = float(np.nanmax(finite))
                else:
                    val = np.nan
                row[pos] = val
                pos += 1
            row[pos] = compute_run_length_since_nonzero(outages, t, loc, lookback_steps)
            pos += 1
            ...
            for feat_idx, feat_name in zip(hazard_idx, hazard_names):
                if t >= 1:
                    ...
                    val = float(lag1) if np.isfinite(lag1) else np.nan
                else:
                    lag1 = np.nan
                    val = np.nan
                row[pos] = val
                hazard_lag_vals[feat_name] = val
                pos += 1
                if t >= 2:
                ...
                for window in active_weather_roll_max:
                    start_idx = t - window
                    if start_idx >= 0:
                        vals = weather[start_idx:t, loc, feat_idx]
                        finite = np.where(np.isfinite(vals), vals, np.nan)
                        val_roll = float(np.nanmax(finite))
                    else:
                        val_roll = np.nan
                    row[pos] = val_roll
                    pos += 1
                for window in active_weather_roll_sum:
                    ...
                    if start_idx >= 0:
                        ...
                        val_roll = float(np.nansum(finite))
                    else:
                        val_roll = np.nan
                    row[pos] = val_roll
                    pos += 1
                lag_val = hazard_lag_vals.get(feat_name, np.nan)
                if np.isfinite(out_lag1_val) and np.isfinite(lag_val):
                    row[pos] = float(out_lag1_val * lag_val)
                else:
                    row[pos] = np.nan
                pos += 1
            row[pos] = hour_sin[t]
...
def assign_count_classes(values: np.ndarray, counts: Sequence[int]) -> np.ndarray:
    ints = np.rint(np.clip(values, 0.0, None)).astype(np.int64)
    tail_idx = len(counts)
    classes = np.full_like(ints, tail_idx, dtype=np.int32)
    for idx, count in enumerate(counts):
        classes[ints == count] = idx
    return classes


def calibrate_prob_matrix(
    probs: np.ndarray,
    ...
    nonzero_idx = [i for i in range(num_classes) if i != zero_idx]
    nonzero = adjusted[:, nonzero_idx].sum(axis=1)
    calibrated = calibrator.transform(nonzero)
    calibrated = np.clip(calibrated, 0.0, 1.0)
    tiny_mask = nonzero < 1e-6
    good_mask = ~tiny_mask
    if np.any(good_mask):
        raw = np.clip(nonzero[good_mask], 1e-6, None)
        scale = calibrated[good_mask] / raw
        adjusted[np.ix_(good_mask, nonzero_idx)] *= scale[:, None]
        adjusted[good_mask, zero_idx] = 1.0 - calibrated[good_mask]
    if np.any(tiny_mask):
        adjusted[np.ix_(tiny_mask, nonzero_idx)] = 0.0
        adjusted[tiny_mask, zero_idx] = 1.0 - calibrated[tiny_mask]
        adjusted[tiny_mask, tail_idx] = calibrated[tiny_mask]
    return adjusted
...
def predict_probabilities(
    ...
    if isinstance(probs, list):  # lightgbm can return list per class
...
def expected_outage(
    ...
    discrete = probs[:, : len(counts)]
    base = discrete @ weights
    tail_prob = probs[:, len(counts)]
    return base + tail_prob * tail_pred


def rolling_forecast_nstage(
    ...
    for idx, block_start in enumerate(range(0, total_steps, lookahead_steps), start=1):
        ...
        for t in block_indices:
            ...
            if len(X_step) == 0 or not meta:
                continue
            raw_probs = predict_probabilities(classifier, X_step)
            raw_nonzero = 1.0 - raw_probs[:, zero_idx]
            adj_probs = calibrate_prob_matrix(raw_probs, calibrator, zero_idx, tail_idx)
            tail_pred = np.clip(regressor.predict(X_step), 0.0, None)
            preds = expected_outage(adj_probs, tail_pred, counts)
            p_nonzero = 1.0 - adj_probs[:, zero_idx]
            for (time_idx, loc_idx), truth, pred, pnz in zip(meta, y_step, preds, p_nonzero):
                records.append(
                    {
            ...
            raw_nonzero_all.extend(raw_nonzero.tolist())
    ...
    return (
        df,
        np.asarray(raw_nonzero_all, dtype=np.float32),
        np.asarray(occ_all, dtype=np.int8),
        block_lengths,
    )
...
def train_classifier(
    ...
    class_count = len(counts) + 1  # tail bucket
    clf = lgb.LGBMClassifier(
    ...
    eval_set = [(X_val, y_val_cls)] if len(X_val) else None
    clf.fit(
        X_train,
        y_train_cls,
        eval_set=eval_set,
    ...
    if calibrate and len(X_val) and len(np.unique(val_occ)) > 1:
        val_probs = clf.predict_proba(X_val)
        if isinstance(val_probs, list):
            val_probs = np.stack(val_probs, axis=1)
        zero_idx = counts.index(0)
        p_nonzero = 1.0 - val_probs[:, zero_idx]
        calibrator = IsotonicRegression(out_of_bounds="clip")
        calibrator.fit(p_nonzero, val_occ.astype(float))
...
def train_regressor(
    ...
    seed: int,
    device: str,
    tail_mask_train: np.ndarray,
    tail_mask_val: np.ndarray,
) -> lgb.LGBMRegressor:
    if tail_mask_train.sum() == 0:
        raise RuntimeError("No samples exceeded the discrete count set; cannot fit tail regressor.")
    ...
    eval_set = [(X_val[tail_mask_val], y_val[tail_mask_val])] if tail_mask_val.sum() else None
    reg.fit(
        X_train[tail_mask_train],
        y_train[tail_mask_train],
        eval_set=eval_set,
        eval_metric="rmse",
        categorical_feature=[cat_index],
        callbacks=[lgb.early_stopping(200, verbose=False)],
...
def save_calibration_plot(
    ...
    ax.scatter(raw_probs, y_occ, s=8, alpha=0.15, color="tab:orange", label="Samples")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-dir", type=Path, default=Path("results") / "n_stage")
    ...
    parser.add_argument("--counts", type=str, default="0,1,2", help="Comma-separated outage counts to model explicitly (default: 0,1,2).")
    parser.add_argument("--ensemble-size", type=int, default=1, help="Number of ensemble members to train.")
    parser.add_argument("--disable-calibration", action="store_true", help="Skip isotonic calibration.")
...
def main() -> None:
    ...
    counts_list = parse_counts_arg(args.counts)
    zero_idx = counts_list.index(0)
    tail_idx = len(counts_list)
    ...
    y_train_cls = assign_count_classes(y_train, counts_list)
    y_val_cls = assign_count_classes(y_val, counts_list)
    tail_mask_train = y_train_cls == tail_idx
    tail_mask_val = y_val_cls == tail_idx
    ...
    ensemble_raw_nonzero: List[np.ndarray] = []
    ...
    for member in range(ensemble_size):
        ...
        raw_val_probs = predict_probabilities(classifier, X_val)
        val_probs = calibrate_prob_matrix(raw_val_probs, calibrator, zero_idx, tail_idx)
        val_intensity = np.clip(regressor.predict(X_val), 0.0, None)
        _ = expected_outage(val_probs, val_intensity, counts_list)

        df_member, raw_test_nonzero_member, occ_member, block_lengths_member = rolling_forecast_nstage(
            classifier,
            calibrator,
            regressor,
            outages,
        ...
        ensemble_raw_nonzero.append(raw_test_nonzero_member)
    ...
    raw_test_nonzero = np.mean(ensemble_raw_nonzero, axis=0)
    ...
    save_calibration_plot(test_occ_array, raw_test_nonzero, calibrator, calib_path, args.dataset)
